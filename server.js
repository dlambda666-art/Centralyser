...
function sourceCatalogUrl(addon, type, catalogId, query) {
  const base = sourceBaseUrl(addon.manifestUrl);
  const url = new URL(`${base}/catalog/${encodeURIComponent(type)}/${encodeURIComponent(catalogId)}.json`);
...
function sourceMetaUrl(addon, type, id, query) {
  const base = sourceBaseUrl(addon.manifestUrl);
  const url = new URL(`${base}/meta/${encodeURIComponent(type)}/${encodeURIComponent(id)}.json`);
  for (const [key, value] of query) url.searchParams.set(key, value);
  return url.toString();
}
...
async function buildManifest() {
  ...
  for (const addon of config.addons) {
    try {
      ...
      console.error(`[manifest] ${addon.name}: ${error.message}`);
    }
  }
...
function findAddon(id) {
  return config.addons.find((addon) => addon.id === id);
}
...
function sendJson(res, status, data) {
  ...
  res.writeHead(status, {
    ...
    "access-control-allow-headers": "content-type,x-centralyser-key",
...
async function addAddon(){const url=document.getElementById('url').value.trim();const name=document.getElementById('name').value.trim();if(!url)return alert('Entre une URL de manifest.');try{const a=await api('/api/addons',{method:'POST',body:JSON.stringify({url,name})});document.getElementById('url').value='';document.getElementById('name').value='';document.getElementById('addmsg').textContent=' ✓';render((await api('/api/addons')).addons)}catch(e){alert(e.message)}}
...
async function handleApi(req, res, parts, requestUrl) {
  ...
  if (req.method === "GET" && parts.length === 2) {
    return sendJson(res, 200, { addons: await Promise.all(config.addons.map((a) => addonInfo(a))) });
  }
  ...
  if (req.method === "POST" && parts.length === 2) {
    ...
    manifestCache.set(addon.id, { time: Date.now(), data: manifest });
    await saveConfig();
    return sendJson(res, 201, await addonInfo(addon));
  }

  if (parts.length >= 3) {
    const addonId = parts[2];
    const addon = findAddon(addonId);
    if (!addon) return sendJson(res, 404, { error: "Addon not found" });

    if (req.method === "DELETE" && parts.length === 3) {
      config.addons = config.addons.filter((a) => a.id !== addonId);
      manifestCache.delete(addonId);
      await saveConfig();
      return sendJson(res, 200, { ok: true });
    }

    if (req.method === "POST" && parts[3] === "refresh") {
      const manifest = validateManifest(await fetchJson(addon.manifestUrl));
      const valid = new Set((manifest.catalogs || []).map((c) => c.id));
      addon.selectedCatalogs = (addon.selectedCatalogs || []).filter((id) => valid.has(id));
      manifestCache.set(addon.id, { time: Date.now(), data: manifest });
      await saveConfig();
      return sendJson(res, 200, await addonInfo(addon, true));
    }

    if (req.method === "PUT" && parts.length === 3) {
      const body = await readBody(req);
      if (body.name !== undefined) addon.name = String(body.name || addon.name);
      if (body.url !== undefined) {
        const url = String(body.url || "").trim();
        if (!/^https?:\\/\\//i.test(url)) throw new Error("Manifest URL must start with http:// or https://");
        addon.manifestUrl = url;
        manifestCache.delete(addon.id);
        await getAddonManifest(addon, true);
      }
      if (Array.isArray(body.selectedCatalogs)) addon.selectedCatalogs = [...new Set(body.selectedCatalogs.map(String))];
      await saveConfig();
      return sendJson(res, 200, await addonInfo(addon));
    }
  }
...
const server = http.createServer(async (req, res) => {
  if (req.method === "OPTIONS") {
    res.writeHead(204, { "access-control-allow-origin": "*", "access-control-allow-methods": "GET,POST,PUT,DELETE,OPTIONS", "access-control-allow-headers": "content-type,x-centralyser-key" });
    return res.end();
  }
  ...
  try {
    ...
    if (requestUrl.pathname === "/health" && req.method === "GET") return sendJson(res, 200, { status: "ok", addon: "Centralyser", configuredAddons: config.addons.length });
    ...
    if (parts[0] === "catalog" && parts.length === 3 && parts[2].endsWith(".json")) {
      ...
      return sendJson(res, 200, await fetchJson(sourceCatalogUrl(addon, type, parsed.catalogId, requestUrl.searchParams)));
    }
    ...
    if (parts[0] === "meta" && parts.length === 3 && parts[2].endsWith(".json")) {
      ...
      for (const addon of config.addons) {
        try {
          ...
          if (data?.meta || (Array.isArray(data?.metas) && data.metas.length)) return sendJson(res, 200, data);
