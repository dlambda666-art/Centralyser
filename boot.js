...
try {
  ...
  // UI: show all catalogs detected for each addon. The user remains in control
  // of which addons exist; Centralyser never auto-adds an addon.
  ...
  // New addons expose every catalog immediately.
  ...
  // Restore the user's saved addon/catalog state after a server restart.
  ...
  // Keep the Nuvio manifest URL easy to copy.
  ...
  // Retry transient upstream failures. This version deliberately avoids nested
  // template literals so boot.js cannot fail with an interpolation error.
  ...
  // Short response cache to absorb duplicate catalog/meta bursts from Nuvio.
  ...
  // Public manifest: expose every catalog belonging to an addon the user
  // actually configured. It does not invent or add addons.
