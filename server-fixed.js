...
function render(as){const box=document.getElementById('addons');document.getElementById('count').textContent=as.length?'('+as.length+')':'';if(!as.length){box.innerHTML='<p class="muted">Aucun addon configuré pour le moment.</p><p class="muted">Ajoute ton premier addon ci-dessus.</p>';return}box.innerHTML=as.map(a=>{const m=a.manifest||{};const sel=new Set(a.selectedCatalogs||[]);const cats=(m.catalogs||[]).map(c=>'<label class="catalog"><input type="checkbox" '+(sel.has(c.id)?'checked':'')+' onchange="toggle(\''+a.id+'\',\''+esc(c.id)+'\',this.checked)"> '+esc(c.name||c.title||c.id)+' <span class="muted">('+esc(c.type||'')+')</span></label>').join('');return '<div class="addon ok"><h3>✅ '+esc(a.name||m.name||a.manifestUrl)+'</h3><div class="muted">'+esc(a.manifestUrl)+'</div><p>Manifest lu avec succès · '+((m.catalogs||[]).length)+' catalogue(s) détecté(s).</p><div class="row"><button onclick="all(\''+a.id+'\',true)">Tout sélectionner</button><button onclick="all(\''+a.id+'\',false)">Tout désélectionner</button><button onclick="refresh(\''+a.id+'\')">Actualiser</button><button onclick="edit(\''+a.id+'\')">Modifier URL</button><button onclick="del(\''+a.id+'\')">Supprimer</button></div>'+cats+'</div>'}).join('')}
...
http.createServer(async (req,res)=>{
  ...
  try {
    ...
    if(p[0]==='api'&&p[1]==='addons'){
      ...
      if(req.method==='POST'&&p.length===2){const b=await body(req),url=normalizeManifestUrl(b.url),a={id:randomUUID().replaceAll('-','').slice(0,12),name:String(b.name||'').trim()||url,manifestUrl:url,selectedCatalogs:[]},m=validManifest(await fetchJson(url));a.manifest=manifestInfo(m,a);cache.set(a.id,{time:Date.now(),data:m});config.addons.push(a);await saveConfig();return json(res,201,{...a,error:null})}
...
async function buildManifest(){const catalogs=[],types=new Set();for(const addon of config.addons){try{const m=cache.has(addon.id)?cache.get(addon.id).data:(addon.manifest||await getManifest(addon));(m.types||[]).forEach(t=>types.add(t));for(const c of m.catalogs||[])if((addon.selectedCatalogs||[]).includes(c.id))catalogs.push({...c,id:`centralyser__${addon.id}__${c.id}`})}catch(e){console.error(`[manifest] ${addon.name}: ${e.message}`)}}return{id:'com.dlambda.centralyser',version:'1.0.0',name:'Centralyser',description:'Hub personnel configurable de catalogues Stremio.',resources:['catalog','meta'],types:[...types],catalogs}}
