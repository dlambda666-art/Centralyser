...
function normalizeManifestUrl(value) {
  ...
  if (!/^https?:\/\//i.test(raw)) throw new Error("Manifest URL must start with http:// or https://");
...
async function fetchJson(url, timeout = MANIFEST_TIMEOUT) {
  ...
  try {
    ...
    if (e.name === "AbortError") throw new Error(`Timeout après ${timeout / 1000}s: ${url}`);
    throw e;
...
function body(req) { return new Promise((resolve, reject) => { let s=""; req.on("data", c => { s += c; if (s.length > 1000000) req.destroy(new Error("Request too large")); }); req.on("end", () => { try { resolve(s ? JSON.parse(s) : {}); } catch { reject(new Error("Invalid JSON")); } }); req.on("error", reject); }); }
...
function page() { return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Centralyser</title><style>body{font-family:system-ui;margin:0;background:#111827;color:#f9fafb}main{max-width:900px;margin:auto;padding:24px}section{background:#1f2937;border-radius:14px;padding:18px;margin:16px 0}input{width:100%;box-sizing:border-box;padding:12px;border-radius:8px;border:1px solid #4b5563;background:#111827;color:white;margin:6px 0 12px}button{padding:9px 13px;border:0;border-radius:8px;cursor:pointer;margin:3px}button.primary{background:#60a5fa}.addon{border:1px solid #374151;border-radius:10px;padding:14px;margin:10px 0}.catalog{display:block;padding:8px 0;border-top:1px solid #374151}.muted{color:#9ca3af}.row{display:flex;gap:8px;flex-wrap:wrap}a{color:#93c5fd}</style></head><body><main><h1>🧩 Centralyser</h1><p class="muted">Ajoute tes addons, choisis les catalogues à exposer dans Nuvio, puis modifie-les quand tu veux.</p><section><h2>Ajouter un addon</h2><label>Nom (facultatif)</label><input id="name" placeholder="Ex. FrankenStream"><label>URL du manifest</label><input id="url" placeholder="https://... ou https://.../manifest.json"><button class="primary" onclick="add()">Ajouter et lire le manifest</button></section><section><h2>Mes addons</h2><div id="addons">Chargement…</div></section><section><h2>BetterPoster</h2><p class="muted">Activé automatiquement pour les fiches possédant un identifiant IMDb.</p></section><section><h2>Manifest Centralyser</h2><p><a href="/manifest.json" target="_blank">/manifest.json</a></p><p class="muted">À installer une seule fois dans Nuvio.</p></section></main><script>
const key=localStorage.getItem('centralyserKey')||'';const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
async function api(p,o={}){o.headers={...(o.headers||{}),'content-type':'application/json','x-centralyser-key':key};const ctl=new AbortController();const t=setTimeout(()=>ctl.abort(),12000);o.signal=ctl.signal;try{const r=await fetch(p,o);const d=await r.json();if(!r.ok)throw Error(d.error||r.statusText);return d}finally{clearTimeout(t)}}
async function load(){const box=document.getElementById('addons');box.textContent='Chargement…';try{const d=await api('/api/addons');render(d.addons)}catch(e){box.innerHTML='<p>Erreur de chargement : '+esc(e.message)+'</p><button onclick="load()">Réessayer</button>'}}
function render(as){const box=document.getElementById('addons');if(!as.length){box.innerHTML='<p class="muted">Aucun addon configuré. Ajoute ton premier manifest ci-dessus.</p>';return}box.innerHTML=as.map(a=>{const m=a.manifest||{};const sel=new Set(a.selectedCatalogs||[]);return '<div class="addon"><h3>'+esc(a.name||m.name||a.manifestUrl)+'</h3><div class="muted">'+esc(a.manifestUrl)+'</div>'+(a.error?'<p>⚠️ '+esc(a.error)+'</p>':'<p>'+esc(m.description||'')+'</p>')+'<div class="row"><button onclick="all(\''+a.id+'\',true)">Tout sélectionner</button><button onclick="all(\''+a.id+'\',false)">Tout désélectionner</button><button onclick="refresh(\''+a.id+'\')">Actualiser</button><button onclick="edit(\''+a.id+'\')">Modifier URL</button><button onclick="del(\''+a.id+'\')">Supprimer</button></div>'+((m.catalogs||[]).map(c=>'<label class="catalog"><input type="checkbox" '+(sel.has(c.id)?'checked':'')+' onchange="toggle(\''+a.id+'\',\''+esc(c.id)+'\',this.checked)"> '+esc(c.name||c.title||c.id)+' <span class="muted">('+esc(c.type)+')</span></label>').join(''))+'</div>'}).join('')}
async function add(){const url=document.getElementById('url').value.trim();const name=document.getElementById('name').value.trim();if(!url)return alert('Entre une URL de manifest.');try{await api('/api/addons',{method:'POST',body:JSON.stringify({url,name})});document.getElementById('url').value='';document.getElementById('name').value='';load()}catch(e){alert(e.message)}}
async function toggle(id,c,on){const a=(await api('/api/addons')).addons.find(x=>x.id===id);const s=new Set(a.selectedCatalogs||[]);on?s.add(c):s.delete(c);await api('/api/addons/'+id,{method:'PUT',body:JSON.stringify({selectedCatalogs:[...s]})});load()}
async function all(id,on){const a=(await api('/api/addons')).addons.find(x=>x.id===id);const ids=on?(a.manifest.catalogs||[]).map(c=>c.id):[];await api('/api/addons/'+id,{method:'PUT',body:JSON.stringify({selectedCatalogs:ids})});load()}
async function refresh(id){await api('/api/addons/'+id+'/refresh',{method:'POST'});load()}
async function edit(id){const a=(await api('/api/addons')).addons.find(x=>x.id===id);const u=prompt('Nouvelle URL du manifest',a.manifestUrl);if(u&&u!==a.manifestUrl){await api('/api/addons/'+id,{method:'PUT',body:JSON.stringify({url:u})});load()}}
async function del(id){if(confirm('Supprimer cet addon ?')){await api('/api/addons/'+id,{method:'DELETE'});load()}}
...
http.createServer(async (req,res)=>{const u=new URL(req.url,`http://${req.headers.host||"localhost"}`);const p=u.pathname.split('/').filter(Boolean);try{
  ...
  if(p[0]==='api'&&p[1]==='addons'){
    ...
    if(p.length>=3){const id=p[2],a=findAddon(id);if(!a)return json(res,404,{error:'Addon not found'});if(req.method==='DELETE'&&p.length===3){config.addons=config.addons.filter(x=>x.id!==id);cache.delete(id);await saveConfig();return json(res,200,{ok:true})}if(req.method==='POST'&&p[3]==='refresh'){const m=validManifest(await fetchJson(a.manifestUrl));cache.set(id,{time:Date.now(),data:m});a.selectedCatalogs=(a.selectedCatalogs||[]).filter(x=>m.catalogs.some(c=>c.id===x));await saveConfig();return json(res,200,await info(a))}if(req.method==='PUT'&&p.length===3){const b=await body(req);if(b.name!==undefined)a.name=String(b.name||a.name);if(b.url!==undefined){a.manifestUrl=normalizeManifestUrl(b.url);cache.delete(id);await getManifest(a,true)}if(Array.isArray(b.selectedCatalogs))a.selectedCatalogs=[...new Set(b.selectedCatalogs.map(String))];await saveConfig();return json(res,200,await info(a))}}
