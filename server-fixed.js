...
http.createServer(async (req,res)=>{
  ...
  try {
    ...
    if(p[0]==='catalog'&&(p.length===3||p.length===4)&&p[2].endsWith('.json')){const m=/^centralyser__([^_]+)__(.+)$/.exec(decodeURIComponent(p[2].slice(0,-5)));if(!m)return json(res,404,{error:'Catalogue inconnu'});const a=findAddon(m[1]);if(!a)return json(res,404,{error:'Addon inconnu'});const extra=new URLSearchParams(u.searchParams);if(p.length===4&&p[3].endsWith('.json')){for(const [k,v] of new URLSearchParams(decodeURIComponent(p[3].slice(0,-5))))extra.set(k,v);}return json(res,200,better(await fetchJson(endpoint(a,'catalog',p[1],m[2],extra),15000)))}
